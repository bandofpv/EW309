...
pitch_control = Controller(pitch_motor, P=0.12, I=0.6, sampling_rate=sampling_rate, deadzone=[0.19,-0.19])
...
while True:
    ...
    if move_yaw:
        if yaw_control.move_to_angle(wrap2pi(yaw), -5):
            yaw_motor.move(0)
            move_yaw = False
    if move_pitch:
        if pitch_control.move_to_angle(pitch, 0):
            pitch_motor.move(0) 
            move_pitch = False
